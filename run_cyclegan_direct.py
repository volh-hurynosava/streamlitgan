...
def run_test_directly(dataroot, name, checkpoints_dir, results_dir, cyclegan_dir, **kwargs):
    try:
        ...
        print(f"CycleGAN directory: {cyclegan_dir}")
        print(f"CycleGAN exists: {os.path.exists(cyclegan_dir)}")
        
        sys.path.insert(0, cyclegan_dir)
        
        print("Checking CycleGAN module import...")
        ...
        try:
            ...
            print("CycleGAN modules imported successfully")
        except ImportError as e:
            print(f"Error importing CycleGAN modules: {e}")
            print(f"Contents of {cyclegan_dir}:")
            if os.path.exists(cyclegan_dir):
                for item in os.listdir(cyclegan_dir):
                    print(f"  - {item}")
            raise
        
        print(f"Launching CycleGAN with parameters:")
        ...
        print(f"Options configured, creating dataset...")
        
        try:
            dataset = create_dataset(opt)
            print(f"Dataset created, number of elements: {len(dataset)}")
        except Exception as e:
            print(f"Error creating dataset: {e}")
            raise
        
        print(f"Dataset created, creating model...")
        ...
        print(f"Creating results directory: {web_dir}")
        ...
        for i, data in enumerate(dataset):
            ...
            if i % 5 == 0:
                print(f"Processing image ({i:04d})-th... {img_path}")
                
            save_images(webpage, visuals, img_path, aspect_ratio=opt.aspect_ratio, width=opt.display_winsize)
            
        webpage.save()
        
        print(f"CycleGAN completed successfully!")
